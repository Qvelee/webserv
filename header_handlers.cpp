  ...
  while (begin_word != extension.size()) {
	...
	while (extension[end_word] == ';') {
	  ...
	  if (extension[end_word] == DQUOTE) {
		++end_word;
		++begin_word;
		end_word += get_quoted_string(tmp_par.value, extension, begin_word);
	  } else {
...
void content_type(Request &req, std::string const &value) {
  ...
  begin_word += get_token(req.metadata.media_type.type, value, begin_word);
  tolower(req.metadata.media_type.type);
  if (value[begin_word] != '/')
    error(400);
  ++begin_word;
  begin_word += get_token(req.metadata.media_type.subtype, value, begin_word);
  tolower(req.metadata.media_type.subtype);
//  validate_media_type(req.metadata.media_type);

  begin_word += skip_space(value, begin_word, OWS);
  ...
  while (value[begin_word] == ';') {
     ...
     */
    begin_word += get_token(tmp_par.name, value, begin_word);
    if (value[begin_word] != '=')
      error(400);
    ++begin_word;
    //quoted-string / token
    if (value[begin_word] == DQUOTE) {
      ++begin_word;
      begin_word += get_quoted_string(tmp_par.value, value, begin_word);
	  } else {
		begin_word += get_token(tmp_par.value, value, begin_word);
	  }
...
void content_language(Request &req, std::string const &value) {
  ...
  while (value[begin_world] == ',') {
    ++begin_world;
    begin_world += skip_space(value, begin_world, OWS);
  }
